...
class FaceModel:
  def __init__(self, xgpu, xmodel, ximgsz, ):
    if xgpu>=0:
      ctx = mx.gpu(xgpu)
    else:
      ctx = mx.cpu()
    ...
    self.model = None
    if len(xmodel)>0:
      self.model = get_model(ctx, image_size, xmodel, 'fc1')
  ...
  def get_input(self, face_img):
    ...
    if face_img is None:
      return None
    # bbox, points = face_img
    # if bbox.shape[0]==0:
    #   return None
    # bbox = bbox[0,0:4]
    # points = points[0,:].reshape((2,5)).T
    # print("[ag]", bbox, points)
    img = cv2.resize(face_img, self.image_size)
