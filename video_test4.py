...
from centroidtracker2 import PointTracker, CentroidTracker
from intersect import find_intersection
import time

from age_gender.face_model import get_model
import age_gender.face_model as age_gender_detector
...
# age - db detector
ag_model = age_gender_detector.FaceModel(gpuid,'./age_gender/model/model,0','112,112')
im_scale = 1.0
## ------------- RETINA END ----------------
...
#------------------- MAIN PROGRAM --------------------------
		...
		for (objectID, paket) in objects.items():
			...
			face_gua = aslina[bbox[1]:bbox[3], bbox[0]:bbox[2]]
			data_face = ag_model.get_input(face_gua)
			gender, age = ag_model.predict(data_face)
			# --------------------------------
			cv2.rectangle(image, (bbox[0], bbox[1]), 
								 (bbox[2], bbox[3]), 
						 		 (12,255,23), 2)
			cv2.putText(image, f"{age}-{'M' if gender == 1 else 'F'}", 
